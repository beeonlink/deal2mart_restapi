...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your deal2mart_restapi account.<br>
        ...
        &mdash; deal2mart_restapi Team
      `
      return sendMail({ toEmail: email, subject: 'deal2mart_restapi - Password Reset', content })
    })
